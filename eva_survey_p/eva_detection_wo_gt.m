%% Perform evaluation
% Input:
%   ref_line_t                          Detected line segments in the reference image                         
...
function [index_pairs, rep, iou, le, le_a] = eva_detection_wo_gt(ref_line_t, test_line, eval_param)

% Perform bidirectional evaluation
[mat_dist_1, mat_angle_1, mat_iou_1]  = eva_unit(ref_line_t, test_line, eval_param);
[mat_dist_2, mat_angle_2, mat_iou_2]  = eva_unit(test_line, ref_line_t, eval_param);
...
if ~isempty(index_pairs)
    ...
    recall                  = size(index_pairs, 1)/size(ref_line_t, 1);
    rep                     = (precision + recall) ./ 2;
